...
public interface RegistrationRepository extends JpaRepository<Registration, Integer> {
	...
	/*마이페이제이 - 수강 강좌 내역 조회 남동현*/
	    ...
	    List<Registration> findLimitStartIdx(@Param("userId") Integer userId, @Param("start") int start, @Param("size") int size);

	    @Query(value = "SELECT COUNT(*) FROM registration WHERE user_id = :userId", nativeQuery = true)
	    int countRegistrationByUser(@Param("userId") Integer userId);
	}